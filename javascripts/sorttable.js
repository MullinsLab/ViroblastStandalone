...
sorttable = {
  ...
  guessType: function(table, column) {
    ...
    var column_number = table.tBodies[0].rows[0].cells.length;
    for (var i=0; i<table.tBodies[0].rows.length; i++) {
      text = sorttable.getInnerText(table.tBodies[0].rows[i].cells[column]);
   
      if (text != '') {
      	if (column == column_number - 1) {
      		return sorttable.sort_scientific;
      	}else if (text.match(/^-?[£$¤]?[\d,.]+%?$/)) {
          return sorttable.sort_numeric;
        }else if (text.match(/^\d+\/\d+ \(\d+\)$/)) {
          return sorttable.sort_identities;
        }
...
/* commented by wdeng  
  ...
  sort_scientific: function(a,b) {
    var a_arr = new Array();
    var b_arr = new Array();
    var a = parseFloat(a[0]);
    a = a.toExponential(10);
    var b = parseFloat(b[0]);
    b = b.toExponential(10);
    a_arr = a.split(/[Ee]/);  
    b_arr = b.split(/[Ee]/);
    var a1 = a_arr[0];
    var a2 = a_arr[1];
